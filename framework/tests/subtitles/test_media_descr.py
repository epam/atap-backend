from selenium import webdriver

from framework.activity import Activity
from framework.element import Element
from framework.element_locator import ElementLocator
from framework.libs.is_visible import is_visible
from framework.tests.subtitles.lib import elements_with_media, check_for_description


name = '''Ensures that video elements have transcripts and captions'''
WCAG = '1.2.5'
framework_version = 4
webdriver_restart_required = True

elements_type = "video"
test_data = [
    {
        "page_info": {
            "url": "subtitles/page_transcript_ok.html"
        },
        "expected_status": "PASS"
    },
    {
        "page_info": {
            "url": "subtitles/page_transcript_fail.html"
        },
        "expected_status": "FAIL"
    },
    {
        "page_info": {
            "url": "subtitles/page_captions_ok.html"
        },
        "expected_status": "PASS"
    }
]


def test(webdriver_instance: webdriver.Firefox, activity: Activity, element_locator: ElementLocator):
    """
    Test for checking whether pre-recorded video content has audio description.
    Refers to WCAG 2.0 1.2.5.
    ATTENTION: the test does not cover functionality of similar axE tests, but expands them.
    Returns test result as a dict:
    result = {
                'status': <'ERROR', 'PASS' or 'NOTRUN'>,
                'message': <string>,
                'elements': [{'source': Element, 'problem': <string>, 'error_id': <error_id>}, etc.],
                'checked_elements': [<element1>, <element2>, ...],
             }
    """
    result = {'status': 'PASS',
              'message': '',
              'elements': [],
              'checked_elements': []}

    activity.get(webdriver_instance)
    videos = elements_with_media(webdriver_instance, element_locator, ['object', 'embed', 'iframe'], 'video')
    videos += element_locator.get_all_of_type(webdriver_instance, ['video'])

    result['checked_elements'] = videos
    if result['checked_elements']:
        counter = 1

        def check_video(video: Element):
            if not is_visible(video, webdriver_instance):
                return
            nonlocal counter
            print(f'Check video {counter}: {video}')
            counter += 1
            if not check_for_description(element_locator, webdriver_instance, video, 'video', ['transcript']):
                result['elements'].append({'element': video,
                                           'problem': 'Video content seems to be without transcript.',
                                           'error_id': 'Transcript',
                                           'severity': "FAIL"})

            if not check_for_description(element_locator, webdriver_instance, video, 'video', ['captions']):
                result['elements'].append({'element': video,
                                           'problem': 'Video content seems to be without captions.',
                                           'error_id': 'Captions',
                                           'severity': "WARN"})

        Element.safe_foreach(videos, check_video)
        if result['elements']:
            result['status'] = 'FAIL'
            result['message'] = 'Found video elements without transcript/captions'
    else:
        result['status'] = 'NOELEMENTS'
    print(result)
    return result
