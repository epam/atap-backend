...
name = '''Ensures that contrast ratio for all interactive elements (text and non-text) on focus does not violate 
requirements (1.4.3, 1.4.11)'''
WCAG = '1.4.3'
framework_version = 5
...
test_data = [
    {
        "page_info": {
            "url": "contrast/page_good_focus.html"
        },
        "expected_status": "PASS"
    },
    {
        "page_info": {
            "url": "contrast/page_bugs_focus.html"
        },
...
def test(webdriver_instance: webdriver.Firefox, activity: Activity, element_locator: ElementLocator):
    """
    A search is made for all elements without children. We go through the elements, make FOCUS through js and look at
    ...
    return FramesContrast(webdriver_instance, element_locator).result(action='focus')
