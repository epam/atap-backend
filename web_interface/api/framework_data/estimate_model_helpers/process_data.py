...
def transformed_for_model(dataframe: pd.DataFrame, train=True):
    ...
    if train:
        ...
        dataframe[["name", "often_tested", "multirun", "boxcox_lmbda"]].to_csv(
            "estimate_data_helpers/runs_frequency_data.csv"
        )
        ...
        with open(f"{BASE_DIR}/estimate_data_helpers/robust_scaler.pkl", "wb") as rsf:
            pickle.dump(rs, rsf)
        with open(f"{BASE_DIR}/estimate_data_helpers/label_encoder.pkl", "wb") as lef:
            pickle.dump(le, lef)
    else:
...
def _process_for_model_prediction(one_row_frame, numerical_features):
    ...
    dataframe_from_train = pd.read_csv(
        f"{BASE_DIR}/estimate_data_helpers/runs_frequency_data.csv",
        usecols=[1, 2, 3],
    )

    with open(f"{BASE_DIR}/data/label_encoder.pkl", "rb") as lef:
        le = pickle.load(lef)
    with open(f"{BASE_DIR}/data/robust_scaler.pkl", "rb") as rsf:
        rs = pickle.load(rsf)
