...
def estimated_job_runtime_side_effect(args=None, kwargs=None, **options):
    return 10
...
class JobViewSetTestCase(CommonAPITestCase):
    def setUp(self) -> None:
        ...
        self.task_3 = Task.objects.create(
            target_job=self.job, status=Task.SUCCESS, message="testMessage3", test_results=self.test_results
        )
        ...
        self.example = Example.objects.create(
            ...
            issue=self.issue,
            severity="FAIL",
            code_snippet="testCodeSnippet",
            problematic_element_selector="testProblematicElementSelector",
        )
    ...
    # def test_jobs_estimate_job_test_time(self, mock_obj):
    #     url = reverse("api:jobs-estimate-job-test-time")
    #     post_data = {"tests": ["test_example"], "pages": [self.first_page.id, self.second_page.id]}

    #     mock_obj.estimate_job_test_time.return_value = 33
    #     response = self.client.post(url, data=post_data, format="json")
    #     self.assertEqual(response.status_code, status.HTTP_200_OK)
    #     mock_obj.estimate_job_test_time.assert_called_with(
    #         ["test_example"], [self.first_page.id, self.second_page.id]
    #     )
    ...
    #     mock_obj.request_test_for_job.return_value = {"task_id": self.task.id}
    ...
    @mock.patch("web_interface.apps.task.task_functional.estimate_time.calculated_job_runtime")
    def test_jobs_create(self, mock_obj):
        ...
        data = {
            ...
            "vpat_reports_params": [
                {"type": "testType", "standart": "testStandart", "product_type": "testProductType"}
            ],
        }
        mock_obj.side_effect = estimated_job_runtime_side_effect
    ...
    def test_jobs_update(self, mock_obj):
        ...
        mock_obj.update_job_test_time.apply.side_effect = estimated_job_runtime_side_effect

        response = self.client.put(url, data=data, format="json")
        self.job.refresh_from_db()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        mock_obj.update_job_test_time.apply.assert_called_with(kwargs={"job_id": self.job.id})
    ...
    def test_jobs_destroy(self, mock_obj):
        ...
        abort_task_calls = (mock.call(self.task), mock.call(self.task_2))
        mock_obj.assert_has_calls(abort_task_calls, any_order=True)
