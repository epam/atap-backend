...
from web_interface.api.ci_plugin.serializers import (
    CIPluginJobSerializer, CIPluginTaskSerializer, CIPluginJobTaskSerializer, CIPluginAbortTaskTaskSerializer
)
...
class CIPluginJobViewSet(HasKeyForProjectViewABC, mixins.RetrieveModelMixin, GenericViewSet):
    ...
    @swagger_auto_schema(request_body=no_body,
                         responses={status.HTTP_200_OK: CIPluginJobTaskSerializer})
    @action(methods=['POST'], detail=True)
    def start_task(self, request, pk=None):
        job = self.get_object()
        task_id = web_interface.apps.task.tasks.request_test_for_job(job)['task_id']
...
class CIPluginTaskViewSet(HasKeyForProjectViewABC, mixins.RetrieveModelMixin, GenericViewSet):
    queryset = Task.objects.select_related(
        'target_job__project', 'test_results'
    ).prefetch_related('test_results__test_set')
    ...
    @swagger_auto_schema(request_body=no_body,
                         responses={status.HTTP_200_OK: CIPluginAbortTaskTaskSerializer})
    @action(methods=['POST'], detail=True)
    def abort_task(self, request, pk=None):
        web_interface.apps.task.tasks.abort_task(self.get_object())
